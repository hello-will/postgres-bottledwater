...
if [ "$1" = 'help' ]; then
	...
	if [ ! -s "$PGDATA/PG_VERSION" ]; then
		...
		sed -ri 's/#?(max_wal_senders\ =\ ).*/\1'"8"'/' $PGDATA/postgresql.conf
		sed -ri 's/#?(wal_keep_segments\ =\ ).*/\1'"4"'/' $PGDATA/postgresql.conf
		sed -ri 's/#?(max_replication_slots\ =\ ).*/\1'"8"'/' $PGDATA/postgresql.conf
